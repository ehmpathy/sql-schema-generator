...
} from '@src/.test.utils/databaseConnection';
import { getShowCreateTable } from '@src/.test.utils/getShowCreateTable';
  ...
  it('generates tables for an entity with array properties (both updatable and static) and unique on one array, w/ the same syntax as show create', async () => {
    ...
    const home = new Entity({
      ...
      properties: {
        ...
        seller_uuids: prop.ARRAY_OF(prop.UUID()), // the sellers are tracked in a different db
        advert_uuids: {
          ...prop.ARRAY_OF(prop.UUID()), // the adverts are tracked in a different db
          updatable: true, // the advertisements paid for change over time
        },
      },
