...
export const defineInsertVersionIfDynamicDataChangedLogic = ({
  entity,
}: {
  entity: Entity;
}) => {
  ...
  const updateablePropertyWhereClauseConditionals = updatablePropertyNames.map(
    (name) =>
...
IF (v_matching_version_id IS NULL) THEN -- if the latest version record does not match, insert a new version record
  INSERT INTO ${entity.name}_version AS v
    (${
      entity.name
    }_id, created_at, effective_at, ${updatablePropertyColumnNames.join(', ')})
    VALUES
