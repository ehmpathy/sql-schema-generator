...
export const generateTable = ({ tableName, properties, unique }: { tableName: string, properties: { [index: string]: Property }, unique: string[] }) => {
  // 0. validate input
  if (unique.length === 0) throw new Error('must have atleast one unique property; otherwise, idempotency is not guarenteed');

  // 1. define sql per column
  const columnSqls = Object.entries(properties).map(entry => generateColumn({ columnName: entry[0], property: entry[1] }));
  const foreignKeySqls = Object.entries(properties)
    .filter(entity => !!entity[1].references)
    .map((entry, index) => generateConstraintForeignKey({ index, columnName: entry[0], tableName, property: entry[1] }));
